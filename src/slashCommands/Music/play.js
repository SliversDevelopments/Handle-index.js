      ...
      if (search.match(client.Lavasfy.spotifyPattern)) {
        ...
        switch (Searched.loadType) {
         ...
         return await interaction.editReply({embeds: [loadmusic]});
          ...
          const playlistload = new MessageEmbed()
             .setColor(client.embedColor)
             .setTimestamp()
             .setDescription(`Enqueued [${Searched.playlistInfo.name}](${search})\nDuration: [${Searched.tracks.length}`)
        ...
        switch (res.loadType) {
         ...
         const playlistloadds = new MessageEmbed()
             .setColor(client.embedColor)
             .setTimestamp()
             .setDescription(`Playlist added to queue [${res.playlist.name}](${interaction.data.options[0].value}) - \`[${convertTime(res.playlist.duration)}]\``);
